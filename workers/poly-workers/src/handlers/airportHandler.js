...
export default {
  ...
  async checkAirport(env, id) {
    const result = await airportService.checkAirport(env, id);
    if (!result) return ResponseUtil.error("Airport not found", 404);
    return ResponseUtil.success(result, "success", 200);
  },

  async checkAllAirports(env) {
    const result = await airportService.checkAllAirports(env);
    return ResponseUtil.success(result, "success", 200);
  },

  async getAirportNodes(env, id) {
    const result = await airportService.getAirportNodes(env, id);
    return ResponseUtil.success(result, "success", 200);
  }
};
